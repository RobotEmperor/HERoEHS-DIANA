...
void BaseModule::initPoseMsgCallback(const std_msgs::String::ConstPtr& msg) // GUI 에서 init pose topic을 sub 받아 실
{
	...
	{
		init_pose_path = ros::package::getPath("diana_base_module") + "/data/init_pose.yaml";// 로스 패키지에서 YAML파일의 경로를 읽어온다.
		ROS_INFO("FILE LOAD  ::  init_pose");
	}
	else if(msg->data.compare("init_offset_pose") == 0)
	{
		init_pose_path = ros::package::getPath("diana_base_module") + "/data/init_offset_pose.yaml";// 로스 패키지에서 YAML파일의 경로를 읽어온다.
